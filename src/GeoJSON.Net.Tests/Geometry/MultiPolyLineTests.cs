...
{
    [TestFixture]
    public class MultiPolyLineTests : TestBase
    {
        [Test]
        public void Can_Deserialize()
        {
            ...
            var expectMultiPolyLine = GetMultiPolyLine();

            var actualMultiPolyLine = JsonConvert.DeserializeObject<MultiPolyLine>(json);

            Assert.That(actualMultiPolyLine, Is.EqualTo(expectMultiPolyLine));
        }

        private MultiPolyLine GetMultiPolyLine(double offset = 0.0)
        {
            var MultiPolyLine = new MultiPolyLine(new List<PolyLine>
            {
                new PolyLine(new List<MultiLineString>
                {
        ...
        {
            // Arrang
            var polygon1 = new PolyLine(new List<MultiLineString>
            {
                new MultiLineString(new List<LineString> {
                    new LineString(
                        new List<IPosition>{
            ...
            var polygon2 = new PolyLine(new List<MultiLineString>
            {
                //Outer coordinates
                new MultiLineString(new List<LineString> {
                    new LineString(
            ...
            var MultiPolyLine = new MultiPolyLine(new List<PolyLine> { polygon1, polygon2 });
            var expectedJson = GetExpectedJson();

            // Act
            var actualJson = JsonConvert.SerializeObject(MultiPolyLine);
        ...
        public void Equals_GetHashCode_Contract()
        {
            ...
            var left = GetMultiPolyLine(offset);
            var right = GetMultiPolyLine(offset);
